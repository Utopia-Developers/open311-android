...
public class Media {
    ...
    /**
     * Returns the raw, absolute path for an internal Uri For images chosen from
     * the gallery, the Uri must be looked up in the media database in order to
     * get the actual file path. Taking a fresh image using the camera, the app
     * must have created it's own, new file path for the image. Fresh camera
     * images will not be in the media database. Thread Warning: Converting from
     * Uri to a real path requires a database cursor. This function cannot be
     * called from an AsyncTask, as it does its own background processing. It
